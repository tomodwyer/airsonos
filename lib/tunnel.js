...
class DeviceTunnel extends events.EventEmitter {
  static createFor(device, options = {}) {
    ...
    return getZoneAttrs().then(
      zoneAttrs => new DeviceTunnel(device, zoneAttrs.CurrentZoneName, options)
    );
  }
  ...
  bindAirplayServer() {
    ...
    this.airplayServer.on(
      "clientConnected",
      this.handleClientConnected.bind(this)
    );
    this.airplayServer.on(
      "clientDisconnected",
      this.device.stop.bind(this.device, EMPTY_CALLBACK)
    );
  ...
  handleClientConnected(audioStream) {
    ...
    this.airplayServer.on(
      "clientDisconnected",
      this.icecastServer.stop.bind(this.icecastServer)
    );
